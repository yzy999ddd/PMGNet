...
parser = argparse.ArgumentParser(description='Image Deblurring using PANet')

parser.add_argument('--input_dir', default='./test/motion/input', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='./test/motion/restored', type=str, help='Directory for results')
...
with torch.no_grad():
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        for batch in range(len(restored)):
            ...
            utils.save_img((os.path.join(result_dir,  filenames_inp[batch] +'.png')), restored_img)